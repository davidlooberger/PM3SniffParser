...
//        System.out.println(Util.byteArrayToLong(Util.fromHexString("22 18 09 04 0b 00 e0 30 23 07 00 00 00 42 d2 85 4e 23 07 00 00 00 00 21 69 42"), 13, 4));
    ...
    public static String maskPAN(String PAN) {
        ...
        PAN = PAN.replace("F", ""); // Remove filler characters
        String maskedPAN = null;
        if  (PAN.length() == 12)
            maskedPAN = Util.maskCardNumber(PAN, "xxxx-xxxx-####" );
        else if  (PAN.length() == 13)
            maskedPAN = Util.maskCardNumber(PAN, "xxxxx-xxxx-####" );
        else if  (PAN.length() == 14)
            maskedPAN = Util.maskCardNumber(PAN, "xxxx-xxx-xxx-####" );
        else if  (PAN.length() == 15)
            maskedPAN = Util.maskCardNumber(PAN, "####-xxxxxx-x####" );
        else if  (PAN.length() == 16)
            maskedPAN = Util.maskCardNumber(PAN, "####-##xx-xxxx-####" );
        else if  (PAN.length() == 17)
            maskedPAN = Util.maskCardNumber(PAN, "#####-#xxx-xxxx-####" );
        else if  (PAN.length() == 18)
            maskedPAN = Util.maskCardNumber(PAN, "####-##xx-xxxx-x##-###" );
        else if  (PAN.length() == 19)
            maskedPAN = Util.maskCardNumber(PAN, "####-##xx-xxxx-xx##-###" );
        else {
            // Fallback...
            maskedPAN = PAN;
            maskedPAN= StringUtils.overlay(PAN, StringUtils.repeat("X", PAN.length()-4), 0, PAN.length()-4);
        }
        return maskedPAN;
    }
